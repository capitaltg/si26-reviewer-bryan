...
CREATE TABLE "findings" (
	...
	CONSTRAINT "findings_gap_no_proposal" CHECK (("finding_kind" <> 'gap') OR ("proposal_verified" IS NULL AND "evidence_provenance" IS NULL)),
	CONSTRAINT "findings_observation_has_proposal" CHECK (("finding_kind" <> 'observation') OR ("proposal_verified" IS NOT NULL)),
	CONSTRAINT "findings_provenance_iff_proposal" CHECK (("evidence_provenance" IS NOT NULL) = ("proposal_verified" IS TRUE)),
	CONSTRAINT "findings_verified_requires_sides" CHECK (("verification" <> 'verified') OR ("solicitation_verified" AND ("finding_kind" = 'gap' OR "proposal_verified")))
);
...
ALTER TABLE "findings" ADD CONSTRAINT "findings_requirement_id_requirements_id_fk" FOREIGN KEY ("requirement_id") REFERENCES "public"."requirements"("id") ON DELETE set null ON UPDATE no action;
